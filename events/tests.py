...
class UserEventTests(APITestCase):
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(username="testuser2", password="testpassword1234")
        url = reverse('token_obtain_pair')
        resp = self.client.post(url, {"username": "testuser1", "password": "testpassword123"}, format='json')
        ...
        self.event = Event.objects.create(name="Test Event", description="Test Description",
                                          start_date="2023-01-01T00:00:00Z", end_date="2023-01-02T00:00:00Z", creator=self.user1)

    def test_user_can_edit_own_event(self):
        url = reverse('event-detail', kwargs={'pk': self.event.pk})
        data = {"name": "Updated Event", "description": "Updated Description", "start_date": "2023-01-03T00:00:00Z",
    ...
    def test_user_cannot_edit_others_event(self):
        ...
        url = reverse('event-detail', kwargs={'pk': self.event.pk})
        data = {"name": "Attempted Unauthorized Update", "description": "Should Fail", "start_date": "2023-01-03T00:00:00Z",
                "end_date": "2023-01-04T00:00:00Z"}
        response = self.client.put(url, data, format='json')
...
class EventFilterTests(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username="filteruser", password="filterpassword")
        url = reverse("token_obtain_pair")
        ...
        Event.objects.create(name="New Year Party", description="Celebrating the new year", start_date="2023-12-31T00:00:00Z", end_date="2024-01-01T00:00:00Z", creator=self.user)
        Event.objects.create(name="Tech Conference", description="Annual tech conference", start_date="2023-11-15T00:00:00Z", end_date="2023-11-17T00:00:00Z", creator=self.user)
        Event.objects.create(name="Summer Festival", description="Outdoor music and arts festival", start_date="2023-08-05T00:00:00Z", end_date="2023-08-07T00:00:00Z", creator=self.user)
...
class UserEventRegistrationTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.event = Event.objects.create(name="Test Event", description="Test Description",
                                          start_date="2023-01-01T00:00:00Z", end_date="2023-01-02T00:00:00Z", creator=self.user)
